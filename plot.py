...
t = config.getint('diagnostics', 'write_interval') 
...
for i in range(len(ele)):
    ...
    ax.scatter(x_ele, v_ele, s=10, label=f"electron - Timestep: {i*t}") 
    #ax.scatter(x_ion, v_ion, s=5, label=f"ion - Timestep: {i*t}")
    #ax.scatter(x_negion, v_negion, s=10, label=f"negion - Timestep: {i*t}")
    #ax.scatter(x_beam, v_beam, s=10, label=f"beam - Timestep: {i*t}")


    #ax.hist(v_ele, bins=500)
    #ax.set_ylim([0,2500])
