...
fig, (ax_disp, ax_phase1,ax_phase2) = plt.subplots(3, 1, figsize=(10, 8))

f#or i in range(min(len(ele), DATA_TS)):
for i in range(len(ele)):
    j = i*k
    ax_disp.clear()
    ax_disp.plot(x[j], phi[j], color='black',label="potential")
    #ax_disp.plot(x[j], EF[j], color='black',label="E_field")
    #ax_disp.plot(x[j], ndi[j], color='red',label="ndi")
    ax_disp.set_xlabel("x")
    ax_disp.set_ylabel("$\phi$")
    ax_disp.legend()
    ax_disp.set_title(f"Timestep: {j*write_interval}")
    ...
    ax_phase1.set_xlim(ax_disp.get_xlim())
    ...
    # Set common x-axis for both plots
    ax_phase2.set_xlim(ax_disp.get_xlim())
